...
pub struct SyscallData<'a> {
    pub lengths: Vec<f32>,
    pub errors: RayonFnvHashMap<&'a str, Pid>,
}
...
impl<'a> SyscallData<'a> {
    pub fn new() -> SyscallData<'a> {
        SyscallData {
            lengths: Vec::new(),
            errors: RayonFnvHashMap::default(),
        }
    }
}
...
pub struct PidData<'a> {
    pub syscall_data: RayonFnvHashMap<&'a str, SyscallData<'a>>,
...
impl<'a> PidData<'a> {
    pub fn new() -> PidData<'a> {
        PidData {
            syscall_data: RayonFnvHashMap::default(),
            child_pids: Vec::new(),
            open_events: Vec::new(),
            execve: None,
        }
...
pub fn build_syscall_data<'a>(buffer: &'a str) -> RayonFnvHashMap<Pid, PidData<'a>> {
    buffer
        .par_lines()
        .fold(RayonFnvHashMap::default, |mut pid_data_map, line| {
            if let Some(raw_data) = parser::parse_line(line) {
                add_syscall_data(&mut pid_data_map, raw_data);
            }
            pid_data_map
        })
        .reduce(RayonFnvHashMap::default, |mut pid_data_map, temp_map| {
...
fn add_syscall_data<'a>(
    pid_data_map: &mut RayonFnvHashMap<Pid, PidData<'a>>,
    raw_data: RawData<'a>,
) {
...
fn coalesce_pid_data<'a>(
    pid_data_map: &mut RayonFnvHashMap<Pid, PidData<'a>>,
    temp_map: RayonFnvHashMap<Pid, PidData<'a>>,
) {
